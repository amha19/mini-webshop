...
export default class Webbshop extends React.Component<IWebbshopProps, IWebbshopState> {
  ...
  onClickToBasket(id, imgUrl, imgDesc, pris) {
    ...
    this.setState({
      ...
      listTwo: [...this.state.listTwo, {Id: id, ECWS_x002e_ImageUrl: 
  ...
  public render(): React.ReactElement<IWebbshopProps> {
    console.log('Rendering basket list: ', this.state.listTwo);
    console.log("Render ProduktList: ", this.props.produktList);
    console.log("Render OrderList: ", this.state.orderList);
    ...
    return (
      <div className={styles.webbshop}>
        <div className={styles.container}>
          ...
          {this.state.showList ? <div><ul style={{ listStyle: 'none' }}>
            {this.state.listTwo.map((o, index) => {
              return <li><img width={"75px"} height={"75px"} src={o.ECWS_x002e_ImageUrl.Url} />
            ...
            <br></br>
          </ul> <button style={{ float: 'right'}} onClick={this.onClickToOrderList}>Gå till checkout</button></div> : (null)}
          </div>
